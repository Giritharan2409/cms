...
export default function AddStudentModal({ isOpen, onClose, onSuccess, editStudent }) {
  ...
  const initialData = {
    ...
    bloodGroup: '',
    // Academic
    id: `STU-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`,
    ...
    if (isOpen) {
      if (editStudent) {
        ...
        setFormData({
          ...initialData,
          ...editStudent,
          id: editStudent.rollNumber || editStudent.id,
          department: Object.keys(deptMapping).find(key => deptMapping[key] === editStudent.departmentId) || editStudent.departmentId || editStudent.department,
          year: typeof editStudent.year === 'number' ? `${editStudent.year}${editStudent.year === 1 ? 'st' : editStudent.year === 2 ? 'nd' : editStudent.year === 3 ? 'rd' : 'th'} Year` : editStudent.year,
  ...
  const validateStep = (s) => {
    ...
    // Step 4 (Documents) - optional validation
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateStep(step)) {
      try {
        const documents = Object.keys(formData.docs || {}).map(key => ({
          name: key,
          fileName: formData.docs[key]?.name,
          data: formData.docs[key]?.data
        })).filter(d => d.data);
        ...
        const payload = {
          ...
          year: typeof formData.year === 'string' ? parseInt(formData.year.split(' ')[0]) || 1 : formData.year,
          semester: parseInt(formData.semester),
          section: formData.section || 'A',
          status: formData.status || 'active',
          avatar: formData.avatar || null,
          enrollDate: formData.enrollDate || new Date().toISOString().split('T')[0],
          dob: formData.dob || null,
          gender: formData.gender || null,
          address: formData.address || null,
          guardian: formData.guardianName || null,
        ...
        const url = editStudent 
          ? `http://127.0.0.1:5000/api/students/${encodeURIComponent(editStudent.rollNumber || editStudent.id)}`
        ...
        if (!res.ok) {
          ...
          throw new Error(msg || 'Failed to enroll student');
        }
  ...
  const getSemOptions = () => {
    ...
    const yearNum = parseInt(yearStr.split(' ')[0]) || 1;
    ...
    <Modal
      ...
      footer={
        <div className="flex items-center justify-between w-full">
          ...
          <div className={`flex items-center gap-3 ${editStudent ? 'w-full justify-end' : ''}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-right-4">
              ...
              <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {errors.dob && <p className="text-[10px] text-red-500 font-semibold mt-1 ml-1">{errors.dob}</p>}
                </div>
                <div>
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-right-4">
              ...
              <div>
                ...
                <select name="section" value={formData.section} onChange={handleChange} className={inputClasses()}>
                  {['A', 'B', 'C', 'D'].map(s => <option key={s}>{s}</option>)}
                </select>
              </div>
              <div>
                <label className={labelClasses}>Admission Type</label>
                <select name="admissionType" value={formData.admissionType} onChange={handleChange} className={inputClasses()}>
          ...
          {step === 3 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-right-4">
              <div className="md:col-span-2">
                ...
                {errors.guardianName && <p className="text-[10px] text-red-500 font-semibold mt-1 ml-1">{errors.guardianName}</p>}
              </div>
              <div>
              ...
              <div>
                ...
                {errors.guardianPhone && <p className="text-[10px] text-red-500 font-semibold mt-1 ml-1">{errors.guardianPhone}</p>}
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-right-4">
              {[
                ...
                <div key={doc.field} className={`relative border-2 border-dashed rounded-xl p-4 transition-all ${formData.docs[doc.field] ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-200 hover:border-[#1162d4]/50 hover:bg-slate-50'} group`}>
                  <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => handleFileChange(e, doc.field)} />
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${formData.docs[doc.field] ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-100 group-hover:text-blue-600'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-slate-400 truncate">
                        {formData.docs[doc.field] ? formData.docs[doc.field].name : 'Browse file'}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-4">
                <div className="w-16 h-16 rounded-lg overflow-hidden border border-white shadow-sm">
                  <img src={avatarPreview || `https://ui-avatars.com/api/?name=${formData.name}`} alt="Review" className="w-full h-full object-cover" />
                </div>
                <div>
                  <h4 className="text-lg font-bold text-slate-900 leading-none">{formData.name}</h4>
                  <p className="text-xs font-semibold text-[#1162d4] mt-1">{formData.id} â€¢ {formData.department}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-y-4 gap-x-8 text-sm border-t border-slate-100 pt-6">
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Email</p>
                  <p className="font-semibold text-slate-700">{formData.email}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Academic</p>
                  <p className="font-semibold text-slate-700">{formData.year} â€¢ Sem {formData.semester}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Guardian</p>
                  <p className="font-semibold text-slate-700">{formData.guardianName} ({formData.relationship})</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Contact</p>
                  <p className="font-semibold text-slate-700">{formData.guardianPhone}</p>
                </div>
              </div>
            </div>
          )}
